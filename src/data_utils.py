...
def load_and_cache_examples(args, tokenizer,  mode):
    ...
    # args.max_tgt_num_spans = 10 # 3 # MTD default=3 config里面help = type loss eps
    cached_features_file = os.path.join(
        # args.data_dir,
        "/n/holylfs05/LABS/pfister_lab/Lab/coxfs01/pfister_lab2/Lab/coco/ACL/project/data/dataset/jsons_zh/",# 1、zh这里要改
    ...
    #     print("lll")
        ...
        # logger.info("produce cached file %s", cached_features_file)
        ...
        features = convert_examples_to_features(
            ...
            sep_token_extra = False, # bool(args.TOKENIZER_TYPE in ["roberta"]),2、zh这里要改!!!
...
import argparse
# preprocess_zh.161 read_data调用
def get_dep_con1(tokenizer,  mode):
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--data_dir",
        default="/n/holylfs05/LABS/pfister_lab/Lab/coxfs01/pfister_lab2/Lab/coco/ACL/project/data/dataset/jsons_zh/",
        # "data/dataset/jsons_zh/",
        # "/nfs/home/yangliu/code/nlp/jn/secondary/project-DiaASQ/project/data/dataset/jsons_zh/"
        type=str,
        help="",
    )
    parser.add_argument(
    ...
    # zh:# train max len input_ids is 478
    # valid max len input_ids is 485
    # test max len input_ids is 472
    if mode == "train":
        args.max_seq_length = 478
    elif mode== "valid":
        args.max_seq_length = 485
    else:# test
        args.max_seq_length = 472
...
def get_dep_con(tokenizer,  mode,config):
    config.data_dir = "data/dataset/jsons_zh/"
    # config.max_seq_length = 502
    config.local_rank = -1 # help="MTD的多卡模式",
    if mode == "train":
        config.max_seq_length = 478
    elif mode== "valid":
        config.max_seq_length = 485
    else:# test
        config.max_seq_length = 472
    # args.max_tgt_num_spans = 10 # 3 # MTD default=3 config里面help = type loss eps
