...
class Prepare(object):
    def __init__(self, lang):
        ...
        self.opinion_dict = {'Opinion_pos': 'pos', 'Opinion_neg': 'neg', 'Opinion_mid': 'neu',
                             'Opinion_ambiguous': 'amb', 'Opinion_doubt': 'doubt', 'Opinion1_pos': 'pos',
                             "Opinion1_neg": 'neg'}
    ...
    def parse_dialog(self, document):
        ...
        global_dependencies = [[(s2c[(i, u)], w if w == -1 else s2c[(i, w)], x, y, z) for u, w, x, y, z in line] for
                               i, line in enumerate(dependencies)]
        ...
        res = {'doc_id': doc_id, 'sentences': content, 'replies': replies, 'speakers': speakers,
               'sentence_ids': sentence_ids, 'triplets': triples, 'targets': targets, 'aspects': aspects,
               'opinions': opinions, 'dependency': global_dependencies, 'local_dependency': dependencies}
        return res

    def parse_subdialog(self, dialog):

        # Process the dialogue threads and convert the global index to the index in the thread
        doc_id, replies, speakers, sentence_ids, triplets, targets, aspects, opinions, local_dependency, contents = [
            dialog[w] for w in '\
        ...
        for i, line in enumerate(new_sentence_collections):
            ...
            cur_dependency = [[(s2c[(i, u)], w if w == -1 else s2c[(i, w)], x, y, z) for u, w, x, y, z in line] for
                              i, line in enumerate(cur_dependency)]
            cur_dependency = [w for line in cur_dependency for w in line]

            # After obtaining s2c, convert the index of the sentence-level dependency analysis to the index in the entire thread
            cur_line = {'doc_id': cur_doc_id, 'sentences': cur_sentences, 'speakers': cur_speakers, 'triplets': [],
                        'targets': [], 'aspects': [], 'opinions': [], 'dependency': cur_dependency}
            ...
            for line in objects:
                ...
                if c_id0 == -1:
                    ...
                    for k in range(len(sub_dialogs)):
                        ...
                        if not ''.join([w for line in sub_dialogs[k]['sentences'] for w in line][
                                       nstart:nend + 1]) == text.replace(' ', ''):
                            print([w for line in sub_dialogs[k]['sentences'] for w in line][nstart:nend + 1],
                                  text.replace(' ', ''))
                            print(sub_dialogs[k]['doc_id'])
                            xx = 1
                        assert ''.join(
                            [w for line in sub_dialogs[k]['sentences'] for w in line][nstart:nend + 1]) == text.replace(
                            ' ', '')
                        if mode == 'opinions':
                    ...
                    if not ''.join([w for line in sub_dialogs[c_id0]['sentences'] for w in line][
                                   nstart:nend + 1]) == text.replace(' ', ''):
                        print([w for line in sub_dialogs[c_id0]['sentences'] for w in line][nstart:nend + 1],
                              text.replace(' ', ''))
                        print(sub_dialogs[c_id0]['doc_id'])
                        x = 1
                    assert ''.join(
                        [w for line in sub_dialogs[c_id0]['sentences'] for w in line][nstart:nend + 1]) == text.replace(
                        ' ', '')
    ...
    def read_files(self, file_list):
        ...
        def sub_replace(text):
            ...
            text = text.replace('…', '.').replace('“', '"').replace('”', '"').replace('4⃣️', '4..').replace('￣',
                                                                                                            '-').replace(
                '℃', '度')
            return text
        ...
        for file_index, ann_file in enumerate(tqdm(file_list)):
            ...
            # Summarize results
            res.append((
                       ann_file.split('/')[-1][:-4], new_content, reply_speaker, annotation, targets, aspects, opinions,
                       dependencies))
        return res
        ...
        for line in ann_content:
            if line.startswith('T'):
                try:
                    ...
                    tid, (tp, start, end), text = line.split('\t')[0], (
                    line.split('\t')[1].split()[0], *line.split('\t')[1].split(';')[-1].split()), \
                ...
                for w in line_args:
                    if w.startswith('obj'):
                        ...
                        opi_id = w.split(':')[1]
                try:
                    trigger, aspect, opinion = [entities[w] if w is not None else None for w in
                                                [obj_id, asp_id, opi_id]]
                except:
    ...
    def parse_document(self, document):
        ...
        res = []
        url = 'http://{}/'.format(self.config[
                                      'ip_port']) + '?properties={"annotators":"tokenize, ssplit,pos, lemma, depparse","outputFormat":"json", "pipelineLanguage":"zh"}'
