...
class MyDataLoader:
    def __init__(self, cfg):
        ...
        path = os.path.join(cfg.preprocessed_dir,
                            '{}_{}_{}_{}.pkl'.format(cfg.lang, with_test, cfg.bert_path.replace('/', '-'),
                                                     cfg.timestamp))
        ...
        if not os.path.exists(path):
            ...
            data = preprocessor.forward()  # from src_en.preprocess_en import Preprocessor
        ...
        self.kernel = WordPair()  # from src_en.common import WordPair
        self.config = cfg
        self.Count = 0
    ...
    def collate_fn(self, lst):
        ...
        doc_id, input_ids, input_masks, input_segments, sentence_length, token2sents, utterance_index, \
        token_index, thread_length, token2speaker, reply_mask, speaker_mask, thread_mask, pieces2words, new2old, \
        triplets, pairs, entity_list, rel_list, polarity_list, multi_input_ids, multi_input_masks, multi_input_segments, \
        con, dep = zip(*lst)

        dialogue_length = list(map(len, input_ids))

        max_lens = max(map(lambda line: max(map(len, line)), input_ids))  # 应该是这里实现了选择4句话后取最长token然后padding
        ...
        multi_input_ids, multi_input_masks, multi_input_segments = map(multi_padding,
        ...
        for i in con:
            i = i.unsqueeze(0)
            # print(151, i.shape,type(i) )#tensor [1,502]
            ddd.append(i)
        ddd = torch.cat(ddd, dim=0)  # ->(4,502)
        con = torch.tensor(ddd).contiguous()  # .clone().detach()
        # print(151,ddd.shape) # (4,502)
        ...
        for i in dep:
            ...
            # print(151, i.shape,type(i) )#tensor [1,502]
            ddd.append(i)
        ddd = torch.cat(ddd, dim=0)  # ->(4,502,502)
        dep = torch.tensor(ddd).contiguous()
        # print(151,con[0]) # (4,502)
        # print(151,con[1]) # (4,502)
        ...
        res = {
            ...
            "multi_input_ids": multi_input_ids, "multi_input_masks": multi_input_masks,
            "multi_input_segments": multi_input_segments,
            "multi_dialogue_length": multi_dialogue_length,
            "con": con, "dep": dep
        }

        # 把一些参数转换为tensor并放到GPU上
        nocuda = ['thread_lengths', 'pairs', 'doc_id', 'pieces2words', 'new2old', 'con', 'dep']
        res = {k: v if k in nocuda else torch.tensor(v).to(self.config.device) for k, v in res.items()}
        # my_cuda = ['con','dep']
        # print(144,con.shape)# tuple(4,)
        # print(145,dep.shape)

        # res = {k: v if k not in nocuda else torch.tensor(v).to(self.config.device) for k, v in res.items()}
        return res
    ...
    def getdata(self):
        ...
        def load_data(mode):
            ...
            return DataLoader(MyDataset(self.data[mode]), num_workers=0, worker_init_fn=self.worker_init,
                              # batch size为4   shuffle表示每一个 epoch是否为乱序（train时候才）  collate_fn定义如何取样本的
        ...
        for w, z in zip(line, self.data['label_dict']):  # label_dict是字典，在process_en.625
            self.config[w] = z
