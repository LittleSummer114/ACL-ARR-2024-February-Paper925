...
class Span_Classifier_Dep_New(nn.Module):  # BertPreTrainedModel
    def __init__(self, config):
        ...
        self.graph_encoder = GAT(config.hidden_size, config.hidden_size, config.hidden_size, 0.5, 1, 3)
...
class BertWordPair(nn.Module):
    def __init__(self, config):  # config = en_config_roberta_large.yaml
        ...
        bert_config = AutoConfig.from_pretrained(config.bert_path)  # 下载好模型的config

        # 对齐hidden_size 即sequence_outputs的[,,256*3]
        self.policy_hidden_size = bert_config.hidden_size  # config中 1024
        config.policy_hidden_size = bert_config.hidden_size
        self.inputs_shape = bert_config.hidden_size #* 3  # 网络的输入shape  attention的维度问题？
        ...
        self.span_dep_model = Span_Classifier_Dep_New(bert_config)  # 生成3个向量
        ...
        # self.reply_attention = MultiHeadAttention(bert_config.num_attention_heads, bert_config.hidden_size,
        #                                           att_head_size, att_head_size,
        #                                           bert_config.attention_probs_dropout_prob)
        # self.speaker_attention = MultiHeadAttention(bert_config.num_attention_heads, bert_config.hidden_size,
        #                                             att_head_size, att_head_size,
        #                                             bert_config.attention_probs_dropout_prob)
        # self.thread_attention = MultiHeadAttention(bert_config.num_attention_heads, bert_config.hidden_size,
        #                                            att_head_size, att_head_size,
        #                                            bert_config.attention_probs_dropout_prob)
        # self.my_linear = nn.Linear(bert_config.hidden_size, self.inner_dim * 4 * 4) # 这层用于源代码应用注意力机制
    ...
    def classify_matrix(self, kwargs, sequence_outputs, input_labels, masks, mat_name='ent'):
        ...
        else:  # polarity  # self.inner_dim * 4 * 4
    ...
    def forward(self, mode, epsilon,  **kwargs):
        ...
        episode = dict(  # obs根据之前的epoch的actions选择3个向量；obs初始一直是全选！
            # obs=torch.cat([bert_out, graph_out_t, graph_out_s], -1),  # 3*[1，373，1024]->[1，373，1024*3]
            obs = bert_out+graph_out_t+graph_out_s,# [1，373，1024]
        ...
        # print(726,episode['obs'].shape) # torch.Size([1, 318, 3072])
        # for id in range(10):
        #     add = np.random.choice([0, 1, 2, 3, 4, 5, 6, 7])
        #     print(923,add)
        if self.config.ablation==5:# 5-con/6-dep/7-con+dep graph_out_s = con_spans
            ...
            actions = self.policy.choose_actions(episode, epsilon=epsilon,mode=mode)  # 是一维列表(token_num*1)
        ...
        sequence_outputs = self.policy.get_sequence_output(actions, bert_out, graph_out_t, graph_out_s,
                                                           episode['token_num'])# ( 1, 128, 1024 )

        # 根据actions更新obs(调参点)                                                        (episode['token_num'],2,1)
        if update_obs:
        ...
        # 在这里添加计算3个向量代替sequence即可；后面这两行对向量的操作也保留？ 还是说放在前面bert输出后
        if self.config.use_attention:
            sequence_outputs = self.dropout(sequence_outputs) # 如果增加这些，相当于增加对最后向量得到pred的处理而已
        ...
        # 用loss来当r，但是有个问题就是两个agent的r是一样的，之前这里是一个调参点
        # print(217,sequence_outputs.shape) # torch.Size([4, 384, 1024]) (4句话,384个token,每个token的向量表示)  每组4句话不都是pad为384！
        # 几个entity matrix、rel matrix、polarity matrix作为gold
        # entity    tensor(0.0089, device='cuda:0', grad_fn=<NllLossBackward>)
        ...
        print(575, "reward :", r.item())  # 应当逐渐递增 √
        ...
        # actions: (episode['token_num'],2,1)   tags0, tags1, tags2返回计算F1值的
        
        # r = r.unsqueeze(0)  # (1,1)
        # r = r.repeat(episode['token_num'], 1)  # .to(self.device)# (128,1)
        #
        # inputs_RNN = self.policy.get_all_inputs(episode)  # tensor (128,1,1024)
        ...
        # # print(961,inputs_RNN_next.shape,self.policy.eval_hidden.shape)# torch.Size([248, 1, 3072]) torch.Size([4, 1, 8])
        #
        ...
        #                                                         self.policy.eval_hidden)  # 输出 (128,1,hidden_size = 8)
        # self.policy.eval_hidden = self.policy.eval_hidden.data
        ...
        # # print(967,actions)
        # # print(968,actions.shape,q_evals.shape) torch.Size([248, 1, 1]) torch.Size([248, 1, 8])
        # # q_value tensor([[-0.5730,  0.4875,  0.4863,  0.1687, -0.0487, -0.3823,  0.1119,  0.2697],
        # # q_evals = torch.gather(q_evals, dim=2, index=actions).squeeze(2)  # (128,1,8) -> (128,1)
        # # for i in range(episode['token_num']):
        # # print(973,q_evals[i,:])
        # # q_evals = torch.ones((episode['token_num'],1,8))
        # # print(974,q_evals.shape) # torch.Size([248, 1, 8])
        # # print(980,q_evals.shape)
        # # actions=actions.to(self.device)
        # q_evals = q_evals.gather(dim=2, index=actions).squeeze(2)  # (128,1,8) -> (128,1)
        #
        # # print(981,q_evals)
        # # for i in range(episode['token_num']):
        # #     ddd = q_evals[i].gather(dim=1, index=actions[i,:])#.squeeze(2)  # (128,1,8) -> (128,1)
        # #     print(975,ddd)#  torch.Size([248, 1])
        # # for i in range(episode['token_num']):
        # #     print(978, q_targets[i, :])
        # q_targets = q_targets.max(dim=2)[0]  # [0]是因为返回的tuple  (128,1,8) -> (128,1)
        # # print(805,q_evals.device,r.device,q_targets.device)
        #
        ...
        #
        # print("iql loss:", policy_loss.item() )
        #
        # return (loss0, loss1, loss2, policy_loss), (tags0, tags1, tags2), actions
        # actions: (episode['token_num'],2,1)   tags0, tags1, tags2返回计算F1值的