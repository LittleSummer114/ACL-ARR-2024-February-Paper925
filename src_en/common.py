...
class WordPair:
    def __init__(self, max_sequence_len=512):# 初始化各个词典
        self.max_sequence_len = max_sequence_len
        # tgt, asp, opi, the boundary of target, aspect, and opinion term.
        self.entity_dic = {"O": 0, "ENT-T": 1, "ENT-A": 2, "ENT-O": 3}
        # link different types of terms together as a combination: build a triplet of (t_k, a_k, o_k)
        self.rel_dic = {"O": 0, "h2h": 1, "t2t": 2}
        # sentiment predict: build a quard (t_k, a_k, o_k, p_k)
    ...
    def encode_entity(self, elements, entity_type='ENT-T'):
        ...
        entity_list = []
        for line in elements:# "targets": [[20, 21, "iPhone"]
            start, end = line[:2] # self.entity_dic = {"O": 0, "ENT-T": 1, "ENT-A": 2, "ENT-O": 3}
    ...
    def encode_polarity(self, triplets):
        ...
        for triplet in triplets:
            ...
            # 添加entity->opinion的关系  其实是 aspect.start + opinion.start
    ...
    def decode_triplet(self, ent_list, rel_list, pol_list, token2sents):
        ...
        entity_elem_dic = defaultdict(list)# from collections import defaultdict
        entity2type = {}
...
def update_config(config):
    ...
    keys = ['cls', 'sep', 'pad', 'unk', 'bert_path', 'start', 'end']
    for k in keys:
