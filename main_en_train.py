...
# print('toch.__version', torch.__version__)
...
class Main:
    def __init__(self, args):  # 初始化一些dir
        ...
        self.writer = SummaryWriter('other/tensorboard/en_logs/'+"-"+save_name+"-"+timestamp)
        ...
        # print(45,config.device)
        # 强制设置使用cuda 0
        config.device = torch.device('cuda:{}'.format(config.cuda_index))
        # print(47,config.device)
        self.config = config
    ...
    def train_iter(self):  # 输入epoch
        ...
        for i, data in enumerate(train_data):
            # epoch_actions = actions: (episode['token_num'],2,1)  其实没用到传入的self.last_action
            ...
            # 更新last_action 不应该更新！应该一直保持全7
            # self.last_action[i,0:epoch_actions.shape[0],:,:] = epoch_actions
            # print(75,self.last_action[i,0:epoch_actions.shape[0],:,:]) # (episode['token_num'],2,1)
            ...
            description = "Epoch {}, entity loss:{:.4f}, rel loss: {:.4f}, pol loss: {:.4f}, policy_loss:{:.4f}".format(self.global_epoch,*np.mean(losses, 0))

            train_data.set_description(description)
            epsilon = epsilon - (1 - 0.05) / (self.epoch_num - 2)  # 让探索率线性下降到0.05 并且在0.05时候train2个epoch
    ...
    def evaluate_iter(self, dataLoader=None):
        ...
        for i, data in enumerate(dataiter):
            with torch.no_grad():  # self.relation_metric = RelationMetric(self.config)  # from src_en.utils import RelationMetric
    ...
    def evaluate(self, epoch=0, action='eval'):# 'train', 'eval', 'pred'
    ...
    def train(self):
        ...
        for epoch in range(self.config.epoch_size):
            ...
            # from src_en.utils import RelationMetric
            ...
            # break
        if record_loss:
            self.writer.close()# 终端 cd + tensorboard --logdir=logs
    ...
    def forward(self):
        ...
        self.score_manager = ScoreManager()  # 简单的存储score from src_en.common import ScoreManager
        self.relation_metric = RelationMetric(self.config)  # from src_en.utils import RelationMetric
        self.load_param()

        if self.config.action != 'train':# action = 'train', 'eval', 'pred'
        ...
        # if self.best_iter == 0:
        #     return
        record_F(self.config, "en")

if __name__ == '__main__':
    parser = argparse.ArgumentParser()  # 这里设置的值会覆盖en_config里的值
    ...
    parser.add_argument('-random_policy_seed', '--random_policy_seed', type=int, default=0, help='random policy_seed')
    ...
    if args.random_policy_seed:
        args.policy_seed = randint(15, 10000)
        print("use random policy_seed")
    main = Main(args)
    main.forward()
